...
Gate-1 在检测到用友导出的 Excel 财务报表时，自动执行以下已知问题检测：

| 问题 | 检测条件 | 处理方式 | 工具 |
|------|----------|----------|------|
| 权益变动表列错位 | "所有者权益合计"列全为0且"少数股东权益"列有非零值 | 提示并调用修正工具 | `tools/equity_column_fix.py` |
| 键名全角字符 | 报表项目名含全角引号或中文空格导致键名失配 | 解析时自动调用 `norm_key()` 归一化 | 内置于解析逻辑 |

检测到列错位时，Gate-1 输出格式：

```
[Alert] 权益变动表列错位: "所有者权益合计"列全为0，"少数股东权益"列含非零值(期末=X)
  建议: python tools/equity_column_fix.py check <文件路径>
  或: python tools/equity_column_fix.py fix <文件路径> --output <修正后路径>
```
