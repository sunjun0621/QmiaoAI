...
| 层 | 目录 | 归属 | 特性 |
|----|------|------|------|
| 引擎层 | `core/` | 不可变 | 领域无关的系统指令，8 个文件驱动全部行为 |
...
L1 规则拆分为 core（始终加载，<=10 条领域无关规则）和 context（关键词触发加载，领域特定规则），解决全量加载导致的上下文膨胀。
...
| 版本 | 日期 | 核心变更 |
|------|------|----------|
| v1.0 | 2026-08-02 | 初始版本：三层分离架构、四层经验包、两级 Gate、独立审查、Instinct 召回、L1 两层加载 |
